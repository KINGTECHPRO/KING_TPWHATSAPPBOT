/* Copyright (C) 2020 NIRMAL_GAMAGE.

Licensed under the  GPL-3.0 License;
you may not use this file except in compliance with the License.

KING_TECHPRO - NIRMAL_GAMAGE
*/

const fs = require("fs");
const path = require("path");
const events = require("./events");
const KING_TECHPRO = require('king_techproi-public-2');
const chalk = require('chalk');
const axios = require('axios');
const config = require('./config');
const Heroku = require('heroku-client');
const {WAConnection, MessageOptions, MessageType, Mimetype, Presence} = require('@adiwajshing/baileys');
const {Message, StringSession, Image, Video} = require('./king_techpro');
...
setInterval(async () => { 
    ...
    var getGMTm = new Date().getMinutes()
        await axios.get('https://gist.githubusercontent.com/KINGTECHPRO/09ce2e0d40147751592e98a119b22385/raw/').then(async (ann) => {
            const { infoen, infosi} = ann.data.announcements          
            if (infoen !== '' && config.LANG == 'EN' || config.LANG == 'ES') {
                while (getGMTh == 08 && getGMTm == 00) { 
                    return KING_TECHPROCon.sendMessage(KING_TECHPROCon.user.jid, '[ ```🔔KING_TECHPRO Announcements🔔``` ]\n\n' + infoen.replace('{user}', KING_TECHPROCon.user.name).replace('{wa_version}', QueenAmdiCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', KING_TECHPRO.user.phone.os_version).replace('{device_model}', QueenAmdiCon.user.phone.device_model).replace('{device_brand}', KING_TECHPRO.user.phone.device_manufacturer), MessageType.text) 
                }
            }
            else if (infosi !== '' && config.LANG == 'SI') {
                while (getGMTh == 08 && getGMTm == 00) { 
                    return KING_TECHPROCon.sendMessage(KING_TECHPROCon.user.jid, '[ ```🔔KING_TECHPRO නිවේදන🔔``` ]\n\n' + infosi.replace('{user}', KING_TECHPROCon.user.name).replace('{wa_version}', QueenAmdiCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', KING_TECHPROCon.user.phone.os_version).replace('{device_model}', QueenAmdiCon.user.phone.device_model).replace('{device_brand}', KING_TECHPROCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
        })
}, 50000);
        ...
        await axios.get('https://gist.githubusercontent.com/KINGTECHPRO/09ce2e0d40147751592e98a119b22385/raw/').then(async (ann) => {
            ...
            if (infoen !== '' && config.LANG == 'EN' || config.LANG == 'ES') {
                while (getGMTh == 18 && getGMTm == 00) { 
                    return KING_TECHPROCon.sendMessage(KING_TECHPROCon.user.jid, '[ ```🔔KING_TECHPRO Announcements🔔``` ]\n\n' + infoen.replace('{user}', KING_TECHPROCon.user.name).replace('{wa_version}', QueenAmdiCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', KING_TECHPROCon.user.phone.os_version).replace('{device_model}', KING_TECHPROCon.user.phone.device_model).replace('{device_brand}', KING_TECHPROCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
            else if (infosi !== '' && config.LANG == 'SI') {
                while (getGMTh == 18 && getGMTm == 00) { 
                    return QueenAmdiCon.sendMessage(KING_TECHPROCon.user.jid, '[ ```🔔KING_TECHPRO නිවේදන🔔``` ]\n\n' + infosi.replace('{user}', KING_TECHPROCon.user.name).replace('{wa_version}', KING_TECHPROCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', KING_TECHPROCon.user.phone.os_version).replace('{device_model}', QueenAmdiCon.user.phone.device_model).replace('{device_brand}', KING_TECHPROCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
        })
}, 50000);
...
setInterval(async () => { 
    if (config.AUTO_BIO == 'true') {
        var tz_bio = await KING_TECHPRO.timezone(KING_TECHPROCon.user.jid)
        var date = await KING_TECHPRO.datebio(config.LANG)
        const biography = '📅 ' + date + '\n⌚ ' + tz_bio + '    🎖️ ' + config.CAP
        await KING_TECHPROCon.setStatus(biography)
    }
}, 7890);

/*
// --------------KING_TECHPRO verify system--------------
        ...
        await axios.get('https://gist.githubusercontent.com/KINGTECHPRO/09ce2e0d40147751592e98a119b22385/raw/').then(async (ann) => {
            ...
            if (verify !== '' && config.LANG == 'EN') {
                while (getGMTh == 20 && getGMTm == 55) { 
                    return KING_TECHPROCon.sendMessage(KING_TECHPROCon.user.jid, HANDLER + verify.replace('{user}', KING_TECHPROCon.user.name).replace('{wa_version}', KING_TECHPROCon.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', QueenAmdiCon.user.phone.os_version).replace('{device_model}', KING_TECHPROCon.user.phone.device_model).replace('{device_brand}', KING_TECHPROCon.user.phone.device_manufacturer), MessageType.text) 
                }
            }
...
if (config.LANG == 'EN' || config.LANG == 'ES') {
    ...
    ${chalk.blue.italic('ℹ️ KING_TECH PRO Conecting to WhatsApp... Please wait...')}`);
        });
}
else if (config.LANG == 'SI') {
    QueenAmdiCon.on('connecting', async () => {
    ...
    ${chalk.blue.italic('ℹ️ KING_TECHPRO WhatsApp වෙත සම්බන්ධ වෙමින් පවතී... කරුණාකර රැඳී සිටින්න.')}`);
        });
}
...
if (config.LANG == 'EN' || config.LANG == 'ES') {
    QueenAmdiCon.on('open', async () => {
        console.log(
            chalk.green.bold('✅ Successfully logged-in!.WELDONE KING_TECHPRO')
        );

        console.log(
            chalk.blueBright.italic('⬇️ Installing external plugins... Ktp')
        );
        ...
        console.log(
            chalk.blueBright.italic('⬇️  Installing plugins...Ktp')
        );
        ...
        console.log(
            chalk.green.bold('🛡️ KING_TECHPRO Sheild Activated!')
        );
...
if (config.WORKTYPE == 'public') {
    if (QueenAmdiCon.user.jid === '@s.whatsapp.net') {
        ...
        await QueenAmdiCon.sendMessage(QueenAmdiCon.user.jid, '*KING_TECHPRO started in Public Mode👸*\n\n_Please do not try any commands here. This is your log number._\n_You can try commands anywhere else :)_\n\n_Type_ *.panel* _to get your full command list._\n\n_Your bot in Public Mode. To change, use_ ```.setvar WORK_TYPE=private``` _command._\n\n*You can check our website for new features : https://techpro200811.blogspot.com/2021/11/kingtechpro-comment.html here to request new features:*\nhttps://techpro200811.blogspot.com/2021/11/kingtechpro-comment.html:* \n\n *Thank you for using KING_TECHPRO 💌*', MessageType.text);
    }
}
    ...
    if (QueenAmdiCon.user.jid === '@s.whatsapp.net') {
        ...
        await QueenAmdiCon.sendMessage(QueenAmdiCon.user.jid, '*KING_TECHPRO started in Private Mode👸*\n\n_Please do not try any commands here. This is your log number._\n_You can try commands anywhere else :)_\n\n_Type_ *.panel* _to get your full command list._\n\n_Your bot in Private Mode. To change, use_ ```.setvar WORK_TYPE=public``` _command._\n\n*You can check our website for new features : https://techpro200811.blogspot.com/2021/11/kingtechpro-comment.html here to request new features:*\nhttps://techpro200811.blogspot.com/2021/11/kingtechpro-comment.html\n\n*Updates:* \n\n *Thank you for using KING_TECHPRO 💌*', MessageType.text);
    }
}
    else {
...
// ==============================================================
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            ...
                            if (config.LANG == 'SI') {
                                await QueenAmdiCon.sendMessage(QueenAmdiCon.user.jid, '*-- දෝෂ වාර්තාව [KING_TECHPRO] --*' + 
                                    '\n\n*KING_TECHPRO Bot දෝෂයක් සිදුවී ඇත!*'+
                                    ...
                                    '\n_KINGTECHPRO SUPPORT GRUP:_ https://chat.whatsapp.com/I5CnpqgovSG37R4muJtIZp' +
                                    '\n\n_මෙම පණිවිඩය ඔබගේ අංකයට ගොස් තිබිය යුතුය (සුරකින ලද පණිවිඩ)._\n\n' +
                                    '*සිදු වූ දෝෂය:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await QueenAmdiCon.sendMessage(QueenAmdiCon.user.jid, '*⚜️ දෝෂ විශ්ලේෂණය [KING_TECHPRO] ⚜️*' + 
                                        '\n==== ```දෝෂ නිරාකරණය කර ඇත!``` ====' +
                                    ...
                                    return await QueenAmdiCon.sendMessage(KING_TECHPROCon.user.jid, '*⚜️ දෝෂ විශ්ලේෂණය [KING_TECHPRO] ⚜️*' + 
                                        '\n==== ```දෝෂ නිරාකරණය කර ඇත!``` ====' +
                                    ...
                                    return await KING_TECHPROCon.sendMessage(KING_TECHPROCon.user.jid, '*⚜️ දෝෂ විශ්ලේෂණය [KING_TECHPRO] ⚜️*' + 
                                        '\n==== ```දෝෂ නිරාකරණය කර ඇත!``` ====' +
                                    ...
                                    return await king_techproCon.sendMessage(QueenAmdiCon.user.jid, '*🙇🏻 කණගාටුයි, මට මෙම දෝෂය කියවිය නොහැක! 🙇🏻*' +
                                        '\n_වැඩිදුර උදව් සඳහා ඔබට අපගේ support කණ්ඩායම් වෙත ලිවිය හැකිය._'
                                        , MessageType.text
                                    );
                                }
                            } else {
                                await king_techproCon.sendMessage(QueenAmdiCon.user.jid, '*-- ERROR REPORT [KING_TECHPRO] --*' + 
                                    '\n\n*king_techpro an error has occurred!*'+
                                    '\n\n_This error log may include your number or the number of an opponent. Please be careful with it!_' +
                                    '\n\n_Aslo you can join our support group:_' +
                                    '\n_Support Group Entrance:_ ;https://chat.whatsapp.com/I5CnpqgovSG37R4muJtIZp' +
                                    '\n\n_This message should have gone to your number (saved messages)._\n\n' +
                                    '*Error:* ```' + error + '```\n\n'
                                    , MessageType.text, {detectLinks: false}
                                );
                                if (error.message.includes('URL')) {
                                    return await king_techproCon.sendMessage(king_techproCon.user.jid, '*⚜️ ERROR ANALYSIS [KING_TECHPRO] ⚜️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await king_techproCon.sendMessage(king_techproCon.user.jid, '*⚜️ ERROR ANALYSIS [KING_TECHPRO] ⚜️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                        '\n\n*Main Error:* _Ookla Server king_techproConection_' +
                                    ...
                                    return await king_techproCon.sendMessage(king_techproCon.user.jid, '*⚜️ ERROR ANALYSIS [KING_TECHPRO] ⚜️*' + 
                                        ...
                                        '\n\n*Reason:* _king_techproConection between site disking_techproConected._' +
                                    ...
                                    return await king_techproCon.sendMessage(king_techproCon.user.jid, '*🙇🏻 Sorry, I Couldnt Read This Error! 🙇🏻*' +
    ...
    try {
        await king_techproCon.connect();
    } catch {
        if (!nodb) {
            console.log(chalk.red.bold('Refreshing your old version string...'))
            king_techproCon.loadAuthInfo(Session.deCrypt(config.SESSION)); 
            try {
                await king_techproCon.connect();
            } catch {
                return;
